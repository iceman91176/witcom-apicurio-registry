...
#parse the arguments
parser = argparse.ArgumentParser(description='Update pom.xml file with dependencies form another pom-file')
parser.add_argument('pomlocation', help='Location on the filesystem of the pom.xml file to update')
parser.add_argument('dependencypomlocation', help='Location on the filesystem of the pom.xml file containing the dependencies')
...
dependencypomlocation=args.dependencypomlocation
...
def create_dependency(param_groupid,param_artifactid,param_version,param_type,param_classifier,param_exclusions):
  ...
  if param_exclusions is not None:
    #just append
    dependency_element.append(param_exclusions)
...
def merge_dependency(pom_et,param_artifactid,param_groupid,param_type,param_version,param_classifier,param_exclusions):
  artifactfound=False
  ...
  for dependency_element in pom_et.findall('pom:dependencies/pom:dependency',pom_ns):
    ...
    checktype=None
    if get_xpath_element_from_tree(dependency_element,'pom:type',pom_ns) is not None: 
    ...
    if param_classifier is not None:
      ...
      checkclassifier = None
    if (checkgroupid == param_groupid and checkartifactid == param_artifactid and (checktype == param_type or param_type is None) and (checkclassifier == param_classifier or param_classifier is None)):
      artifactfound=True
  ...
  if not artifactfound:
    ...
    dependency_element = create_dependency(param_groupid,param_artifactid,param_version,param_type,param_classifier,param_exclusions)
...
dependency_pom_et = get_tree_from_xmlfile(dependencypomlocation)

#loop through all dependencies in external file
for dependency_element in dependency_pom_et.findall('pom:dependencies/pom:dependency',pom_ns):
  groupid = get_xpath_element_from_tree(dependency_element,'pom:groupId',pom_ns).text
  artifactid = get_xpath_element_from_tree(dependency_element,'pom:artifactId',pom_ns).text
  type=None
  if get_xpath_element_from_tree(dependency_element,'pom:type',pom_ns) is not None:
    type = get_xpath_element_from_tree(dependency_element,'pom:type',pom_ns).text
  pomversion=dependency_element.find('pom:version',pom_ns).text
  exclusions=None
  if get_xpath_element_from_tree(dependency_element,'pom:exclusions',pom_ns) is not None:
    exclusions=get_xpath_element_from_tree(dependency_element,'pom:exclusions',pom_ns)  
  #print groupid + ' ' + artifactid + ' ' + type + ' ' + pomversion
  pom_et,pom_et_changed=merge_dependency(pom_et,artifactid,groupid,type,pomversion,None,exclusions)  
